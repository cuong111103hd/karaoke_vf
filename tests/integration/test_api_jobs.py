...
@pytest.mark.anyio
async def test_api_jobs_endpoints(tmp_path, monkeypatch) -> None:
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        with patch("app.api.routes.jobs.start_background_task") as mock_start:
            ...
            response = await client.get("/api/jobs/missing-job-uuid")
            ...
            response = await client.get("/api/files/jobs/completed-job/instrumental")
            assert response.status_code == 200
