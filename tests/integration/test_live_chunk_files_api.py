...
def test_live_chunk_files_api(tmp_path, monkeypatch) -> None:
    ...
    # 1. Create a live job through POST
    with patch("app.jobs.live_manager.run_live_separation"):
        res = client.post("/api/live-jobs", json={
            "youtube_url": "https://youtube.com/watch?v=123"
        })
        assert res.status_code == 201
        job_id = res.json()["job_id"]
        
    # Test case 1: Not-ready chunk (before manifest exists, chunk index doesn't exist) -> HTTP 404
    res = client.get(f"/api/live-jobs/{job_id}/chunks/0/instrumental")
    assert res.status_code == 404
    
    # Write a mock manifest to disk
    ...
    # Test case 2: Success download chunk 0
    res = client.get(f"/api/live-jobs/{job_id}/chunks/0/instrumental")
    assert res.status_code == 200
    assert res.content == b"fake wav audio content"
    
    # Test case 3: Chunk 1 not ready -> HTTP 400
    res = client.get(f"/api/live-jobs/{job_id}/chunks/1/instrumental")
    assert res.status_code == 400
    assert "not ready" in res.json()["detail"].lower()
    
    # Test case 4: Missing chunk index -> HTTP 404
    res = client.get(f"/api/live-jobs/{job_id}/chunks/99/instrumental")
    assert res.status_code == 404
    
    # Test case 5: Unknown job id -> HTTP 404
    res = client.get("/api/live-jobs/unknown-id/chunks/0/instrumental")
    assert res.status_code == 404
    
    # Test case 6: File missing on disk (chunk is ready, but file is deleted)
    inst_file.unlink() # Delete the file
    res = client.get(f"/api/live-jobs/{job_id}/chunks/0/instrumental")
    assert res.status_code == 404
